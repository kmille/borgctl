...
borg\_create\_backup\_dirs:  
  - "/"  
  
borg\_create\_excludes:  
  - ".cache"  
...
envs:  
  "BORG\_RELOCATED\_REPO\_ACCESS\_IS\_OK": "yes"  
  
borg\_binary: "/usr/bin/borg"
```
...
You need [uv](https://docs.astral.sh/uv/getting-started/installation/). Get get started, use `poetry install`. Then you can run `uv run borgctl --help`. The code can be found in the borgctl directory. Deployment/monitoring stuff is in the contrib directory. Tests can be run with `uv run pytest tests -s -v -x`.  To run the mypy checks, run `uv run mypy`. Mypy is configured in `pyproject.toml`.
